...
def reloadComplains(request):
    ...
    complain_query = "with t as(SELECT (SELECT branch_id FROM project_data_branch WHERE id = (SELECT branch_id FROM usermodule_usermoduleprofile WHERE user_id = au.id)) AS brc, pdc.* FROM project_data_complain pdc LEFT JOIN auth_user au ON au.id = pdc.pin_id) select concat(t.brc,lpad(t.id::text, 5, '0')) AS ticket_id, to_char(t.received_time, 'Dy dd Mon YYYY HH24:MI') as received_time, t.account_no, t.service_type, t.execution_status, t.transaction_id, to_char(t.transaction_date_time, 'Dy dd Mon YYYY HH24:MI') as transaction_date_time, t.id from t where (t.execution_status = 'Read' OR t.execution_status = 'New') and t.received_time > '"+str(start_time)+"' and t.received_time < '"+end_time+"' order by t.received_time DESC"
...
def dashboard(request):
    context = RequestContext(request)
    cursor = connection.cursor()
    query1="select execution_status,COUNT(*) as requests  from project_data_complain group by execution_status"

    cursor.execute(query1)
    data_list = dictfetchall(cursor)

    #query2 ="select sum(t) from (select count(*) as t  from project_data_complain where execution_status='Executed' union select count(*) as t from project_data_complain where execution_status='Not Executed')a union select count(*) from  project_data_complain "
    query2='''select sum( request_offered)as ro,sum(avg_handled_time)as aht,sum(request_replied) as rr, sum(max_handled_time)as awt from(select service_type,request_offered,(executed+not_executed) request_replied, executed,not_executed, (request_offered - (executed+not_executed)) request_rectified,max_handled_time, avg_handled_time, min_handled_time from (
select service_type,get_request_count(service_type,'1971-03-26' , '2999-03-26') as request_offered,
sum(executed_count) executed,sum(not_executed_count) not_executed,avg(solvetime) avg_handled_time,max(solvetime) max_handled_time,min(solvetime) min_handled_time from (
select *,getworkingtime(transaction_date_time ,resolvetime) solvetime from (
select id,service_type,execution_status,executed_count,not_executed_count,transaction_date_time,max(change_time) resolvetime from (
select pdc.id,pdc.service_type,pdc.execution_status ,pdc.received_time transaction_date_time,pdcs.change_time,(CASE WHEN pdc.execution_status='Executed' THEN 1  ELSE 0  END) AS executed_count,
(CASE WHEN pdc.execution_status='Not Executed' THEN 1 ELSE 0  END) AS not_executed_count
from project_data_complain pdc inner join project_data_complainstatuslog pdcs on pdc.id = pdcs.complain_id
where (pdc.execution_status = 'Executed' or pdc.execution_status = 'Not Executed') AND pdc.received_time between '1971-03-26' AND '2999-03-26') t
group by id,service_type,execution_status,executed_count,not_executed_count,transaction_date_time) resolve) complain_summary
group by service_type) inception)g
'''

    cursor.execute(query2)
    data_list1 = dictfetchall(cursor)

    chart_query=''' select received_time, count(*) as t from project_data_complain WHERE received_time >=(CURRENT_DATE - INTERVAL '60 day') GROUP BY received_time
'''

    cursor.execute(chart_query)
    data_list2 = dictfetchall(cursor)
    #data_list3= dictfetchall(cursor)
    data_exp_list = []
    data_exp_list2=[]
    # print data_list
    for data in data_list2:
        ...
        data_obj['received_time'] = str(formats.date_format(data['received_time'], "m/d/Y h:i:s"))
        data_obj['t'] = int(data['t'])
        data_exp_list.append(data_obj)

        #data_exp_list.append(int(data['t']))
        #data_exp_list.append(str(formats.date_format(data['received_time'], "m/d/Y h:i:s")))
    ...
    return render_to_response(
        'project_data/dashboard.html',
        {'reports': 'reports', 'service': 'service',
         'page_header': 'Service Report',
         'data_list': data_list,
         'data_list1': data_list1,
         'data_list2':data_exp_list,
...
def report_agents_performance(request):
    ...
    options_query = "select cpsummary.id,pin, account_no, service_type, received_time,execution_status,remarks_of_bkash_cs,change_time, bkash_agent_id,username from( select cp.*,cplog.change_time, bkash_agent_id from(SELECT id,pin_id pin, account_no, service_type, received_time,execution_status,remarks_of_bkash_cs FROM project_data_complain where (execution_status='Executed' or execution_status='Not Executed' or execution_status='Escalated') and received_time between '"+ str(from_date) +"' and '"+ str(to_date) +"') cp, (SELECT complain_id, change_time, bkash_agent_id FROM project_data_complainstatuslog where (status='Executed' or status='Not Executed'))cplog where cp.id=cplog.complain_id) cpsummary,auth_user where auth_user.id=cpsummary.bkash_agent_id "+ options_query_extra + user_filter_query
...
def report_agents_activity(request):
    ...
    options_query = "select username,login_time,logout_time,user_ip,user_browser from  public.usermodule_useraccesslog inner join public.auth_user on public.usermodule_useraccesslog.user_id=public.auth_user.id where login_time>='" + str(from_date) + "' and logout_time<='" + str(to_date) + "' "+options_query_extra
    ...
    return render_to_response(
        ...
        {'acreports': 'acreports', 'agents_activity_report': 'agents_activity_report', 'userlist': userlist,
...
def report_customer_service_status(request):
    from_date = '1971-03-26'
    to_date = '2999-03-26'
    q_objects_first_query = Q(execution_status='Executed') | Q(execution_status='Not Executed') | Q(execution_status='Escalated')
    if request.method == 'POST':
        # from_date = request.POST.get('start', from_date)
        # to_date = request.POST.get('end', to_date)
        service_type = request.POST.get('service_type', 'custom')
    ...
    for complain in complains:
        ...
        hasattr(complain.pin, 'usermoduleprofile') and complain.pin.usermoduleprofile.branch is not None)  else 'N/A'
        ret_obj['region_name'] = complain.pin.usermoduleprofile.region.name if (
        hasattr(complain.pin, 'usermoduleprofile') and complain.pin.usermoduleprofile.region is not None)  else 'N/A'
        ret_obj['service_type'] = complain.service_type
        ret_obj['execution_status'] = complain.execution_status
...
def getLastId(request):
    query = "SELECT max(id) FROM public.project_data_complain"
    cursor = connection.cursor()
    cursor.execute(query)
